    ...
    <div class="max-w-4xl mx-auto">
      ...
      >
        <div class="flex flex-col items-center gap-4">
          ...
          <div class="flex gap-3 flex-wrap justify-center text-1xl">
            <button 
...
async function prepareFolder(relativePath) {
  ...
  try {
    ...
    folder_destino_selec_by_user.value = response.data?.folder_destino_selec_by_user || ''
    folderCache.set(relativePath, folderId )
    console.log(`✅ Carpeta preparada: ${relativePath} -> ${folderId} -> ${folder_destino_selec_by_user.value}`)
    return folderId
  } catch (error) {
...
async function uploadFile(item) {
  ...
  formData.append('folder_destino_selec_by_user', folder_destino_selec_by_user.value)
...
async function cerrar(){
    ...
    if (status.hasPending || status.uploading > 0) {
      ...
      folder_destino_selec_by_user.value=''
